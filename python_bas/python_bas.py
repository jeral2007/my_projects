...
def water_mass(arr_of_pool):
    ...
    while(ind_l+w_l<=ind_r-w_r-1):
            ...
            if (arr_of_pool[cl]>water_lev_l): # if we meet right wall of the left current pit
                water_tot+=water_lev_l*w_l-l_volume #add to the total water amount amount in the current left pit
           ...
           # for the right pit we do the same
            if (arr_of_pool[cr]>water_lev_r): 
                water_tot+=water_lev_r*w_r-r_volume
                ind_r=cr 
                w_r= 1 
    ...
    #DEBUG: print ({"ind_l":ind_l,"ind_r":ind_r,"cl":cl,"cr":cr,
           #"water_lev_l":water_lev_l,"water_lev_r":water_lev_r,
           # "l_volume":l_volume,"r_volume":r_volume,"w_l":w_l,"w_r":w_r})
